...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var zh = {
			...
			unknown: "未检查",
			ok: "知道了",
			checkAllNoUpdate: "当前没有可更新的技能",
			checkAllHasUpdate: "有 {count} 个技能可更新",
			checkAllUpToDate: "所有已接入更新源的技能都已是最新版本。"
		};
		var en = {
			nav: "Skills",
			...
			unknown: "Not checked",
			ok: "OK",
			checkAllNoUpdate: "No skills have updates",
			checkAllHasUpdate: "{count} skills can be updated",
			checkAllUpToDate: "All skills with an update source are up to date."
		};
		...
		function SkillManagerSection({ t, list, remove, trash, undo, purge, checkUpdate, updateSkill }) {
			...
			var checkResultState = react.useState(null);
			var checkResult = checkResultState[0];
			var setCheckResult = checkResultState[1];
			...
			var onCheckAll = function () {
				return function () {
					setBusy("check-all");
					setNotice(null);
					checkUpdate().then(function (payload) {
						var results = payload && payload.results ? payload.results : [];
						var available = results.filter(function (item) { return item.updateAvailable; });
						setCheckResult({ available: available, total: results.length });
			...
			var checkResultModal = checkResult === null ? null : h("div", {
				style: {
					position: "fixed",
					inset: "0",
				...
				h("div", {
					...
					h("div", { style: { fontSize: "14px", fontWeight: 600, color: "var(--dsw-alias-label-primary)" } },
						checkResult.available.length === 0 ? t("checkAllNoUpdate") : t("checkAllHasUpdate", { count: checkResult.available.length })
					),
					checkResult.available.length === 0
						? h("p", { style: { margin: 0, fontSize: "13px", color: "var(--dsw-alias-label-secondary)" } }, t("checkAllUpToDate"))
						: h("ul", {
							style: {
								listStyle: "none",
								margin: 0,
						...
						}, checkResult.available.map(function (item) {
							return h("li", { key: item.path, style: { fontSize: "13px", color: "var(--dsw-alias-label-primary)" } },
								item.name + ": v" + item.version + " → v" + item.latestVersion
							);
						})),
					h("button", {
						type: "button",
						onClick: function () { setCheckResult(null); },
						style: {
							font: "inherit",
							cursor: "pointer",
							alignSelf: "flex-end",
